...
-- ainda nao mexeu em nada, cai no "camba2" padrao.
Img = (storage and storage["_custom_img_name"] or "camba2") .. ".png"
...
if rootWidget then
  ...
  if botWindow then
    ...
    botWindow:setWidth(210)--Largura
...
-- FIX (Slow macro): esse era o macro(1,...) mais pesado do pacote -- alem
-- de rodar a 1ms, faz um loop de getPlayerByName (que varre os spectators)
-- pra cada jogador rastreado em storage.battleTracking. Um timer de PZ/PK
-- nao precisa de precisao de milissegundo; 100ms (10x/s) e mais que
-- suficiente e reduz drasticamente a frequencia desse loop.
