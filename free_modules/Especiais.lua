...
-- FIX (Slow macro): rodava a 1ms so pra acompanhar o skill de medicine,
-- que muda devagar. 100ms elimina o falso alarme sem perder nada.
...
-- FIX (Slow macro): so controla cooldown de uma magia, nao precisa de 1ms.
...
-- FIX (Slow macro): puramente visual (mostrar/esconder um label), nao
-- precisa de 1ms -- 50ms e imperceptivel.
macro(50, function()
	local widget = screenWidget["mainPanel"]["message"];
...
-- FIX (Slow macro): so acompanha a posicao de um widget na tela, 100ms
-- e mais que suficiente e nao gera falso aviso.
local widgetPos;
macro(100, function()
