...
local widgetPos;
-- As macro(1,...) desse arquivo foram ajustadas pra macro(15,...):
-- 15ms e imperceptivel pra combo (bem abaixo do tempo de reacao humano
-- e do delay de qualquer spell), mas reduz bastante a carga acumulada
-- de rodar 12 macros a 1ms (praticamente todo frame) ao mesmo tempo.
macro(15, function()
...
onTextMessage(function(mode, text)
	...
	text = text:lower();
	if not (text:find("due to your") or text:find("you deal") or text:match("perdeu %d+ de hps") or text:find("pelo seu ataque")) then return; end
...
-- FIX (Slow macro - CRITICO): esta e a macro que mais gerava aviso de
-- "Slow macro". Ela roda a cada 50ms (ja throttled de proposito) mas faz
-- trabalho de verdade por tick (recursivecopy, crc32, json.encode e uma
-- simulacao de ate 15s de combate) -- ou seja, quase sempre passa de 1ms
-- de verdade, sem nenhum problema real de performance. O ["timeout"] = 1
-- forcava o aviso mesmo assim. Removido para usar o limite padrao do bot.
