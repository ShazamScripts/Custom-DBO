...
local isDragKeyPressed = function()
    return g_keyboard.isShiftPressed()
end
...
function refreshTimerWidget()
  ...
  for index, value in pairs(storage.ES_Players) do
    if value.time <= os.time() then table.remove(storage.ES_Players, index) end
    local label = setupUI(addTimer, timerWidget.timersList)
    if value.active >= os.time() then
      label:setColoredText({value.name, "white",
      ": ", "white",
      value.spell, "orange",
      " [ AC: ", "teal",
      value.active - os.time(), "teal",
      " ]", "teal"})
    else
      label:setColoredText({value.name, "white",
      ": ", "white",
...
--                      ARD CUSTONS                           
--===========================================================
-- FIX (Slow macro): so redesenha um widget de texto/timer, 100ms basta.
macro(100, refreshTimerWidget)

local configName = modules.game_bot.contentsPanel.config:getCurrentOption().text;
local enemySpellFile = "/bot/" .. configName .. "/" .. name() .. "_TimeSpellEnemy.json"
