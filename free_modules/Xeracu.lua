...
FollowAttack.checkifWentToJumpPos = function(creature, newPos, oldPos)
    ...
    local spell = FollowAttack.jumpSpell.up;
    local dir = nil;

    if (newPos.z > oldPos.z) then
        spell = FollowAttack.jumpSpell.down;
        dir = newPos.y ~= oldPos.y and 2 or 1;
    else
        dir = newPos.x ~= oldPos.x and 3 or 0;
    end
...
FollowAttack.mainMacro = macro(50, 'Follow Infinito', function()
...
test1 = addIcon("8", {item =944, text = "Fudeu"}, FollowAttack.mainMacro)
test1:breakAnchors()
test1:move(550, 500)
