...
-- FIX (Slow macro): rodava a 1ms, o que forcava o aviso "Slow macro: Escadas"
-- a disparar toda vez que o motor levasse mais de 1ms pra chamar essa funcao
-- de novo (o que e sempre, nenhum engine consegue honrar 1ms). 15ms continua
-- imperceptivel pro jogador (66x por segundo) e elimina o falso alarme.
local mainMacro = macro(15, "Escadas", function()
...
-- FIX (Slow macro): mesmo so enfileirando verifyTiles (trabalho pesado
-- ja roda em pedacos via Queue_call), um macro(1,...) ainda e reavaliado
-- a cada tick do engine e pode aparecer como "culpado" quando o tick como
-- um todo demora (varias macros somadas). 15ms reduz essa chance sem
-- atrasar o enfileiramento de forma perceptivel.
macro(15, function()
