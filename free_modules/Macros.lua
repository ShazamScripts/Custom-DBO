...
storage.rtsFollowOn = storage.rtsFollowOn == true

ultimateFollow = macro(350, "Follow RTS", function()
  if not storage.rtsFollowOn then return end
...
ultimateFollow.setOn(storage.rtsFollowOn)

local rtsFollowCheck = setupUI([[
CheckBox
  id: rtsFollowToggle
  text: "Follow RTS Ativo"
  checked: false
]], mainTab)

rtsFollowCheck.onCheckChange = function(widget, checked)
  storage.rtsFollowOn = checked
  if checked then
...
rtsFollowCheck:setChecked(storage.rtsFollowOn)
...
followNormalMacro = macro(200, "Follow Normal", function()
  local center = currentPosition()
  ...
  for _, followcreature in ipairs(spectators) do
    ...
    and getDistanceBetween(center, followcreature:getPosition()) <= 8 then
