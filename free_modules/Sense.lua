...
senseMaintain["macro"] = macro(storage["scrollBars"]["macroDelay"] or 50, "Sense", function()
...
-- FIX (Slow macro): puramente visual (mostra/esconde a seta), 50ms basta.
macro(50, function()
	senseMaintain["pointer"]:hide();
...
if (not runBot) then
	...
	UI["TextEdit"](storage["_custom_img_name"] or "camba2", function(widget, text)
		text = text:trim();
		if (text == "") then
			widget:setText(storage["_custom_img_name"] or "camba2");
			return;
		end
