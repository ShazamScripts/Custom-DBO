...
-- FILTRO DE NIVEL
-- ============================================================
-- A maioria dos servidores (inclusive esse, aparentemente) NAO manda o
-- level de outros players pela rede -- so o SEU proprio level vem no
-- login. Por isso "creature:getLevel()" costuma vir 0/nil pra qualquer
-- player que nao seja voce; isso nao e bug do script, e limitacao do
-- protocolo. O jeito que funciona na pratica e ler o level do texto que
-- aparece quando voce (ou o bot) da "look" no player -- ex: "You see
-- Fulano (Level 150)." -- e guardar isso num cache por nome.
attackEnemy["levelCache"] = attackEnemy["levelCache"] or {};
local levelCache = attackEnemy["levelCache"];
...
onTextMessage(function(mode, text)
	...
	-- cobre "you see NOME (Level 150)." e variacoes com titulo/guild depois,
	-- desde que "(level N)" venha logo depois do nome.
	local name, level = lower:match("^you see ([^%(]-)%s*%(level (%d+)%)");
	if (not name) then
		name, level = lower:match("^you see ([^%(]-)%s*%(n%a+vel (%d+)%)");
	end
	if (name and level) then
		levelCache[normalizeName(name)] = {level = tonumber(level), time = os["time"]()};
		print("[Shazam Scripts] Level descoberto: " .. name:ucwords() .. " = " .. level);
	end
end)

-- Auto-look: quando um player aparece como possivel alvo mas ainda nao
...
local enemyLevelScrollBar = "Panel\n  height: 34\n\n\n  Label\n    id: text\n    anchors.left: parent.left\n    anchors.right: parent.right\n    anchors.top: parent.top\n    text-align: center\n\n  HorizontalScrollBar\n    id: scroll\n    anchors.left: parent.left\n    anchors.right: parent.right\n    anchors.top: prev.bottom\n    margin-top: 3\n    minimum: 0\n    maximum: 1000\n    step: 5\n";

local enemyLevelWidget = setupUI(enemyLevelScrollBar, panel);
enemyLevelWidget["scroll"]:setRange(0, 1000);
enemyLevelWidget["scroll"]:setStep(5);
enemyLevelWidget["scroll"]:setValue(storage["scrollBars"]["enemyMinLevel"] or 150);
enemyLevelWidget["scroll"]["onValueChange"] = function(scroll, value)
	storage["scrollBars"]["enemyMinLevel"] = value;
...
local passesLevelFilter = function(creature, selfName)
	...
	local cached = levelCache[normalizeName(selfName)];
		...
		if (self:isPlayer()) then
			...
			if (healthPercent and healthPercent > 0) then
					if (not attackEnemy["isFriend"](selfName)) then
						if (not passesLevelFilter(self, selfName)) then
							return false;
						end
...
-- FIX (Slow macro): "or 1" era o fallback caso macroDelay nao existisse
-- ainda; trocado por "or 50" pra nunca cair em 1ms.
macro(storage["scrollBars"]["macroDelay"] or 50, function(self)
