-- ============================================================
-- STORAGE SEPARADO POR PERSONAGEM
-- ============================================================
...
-- Se dois personagens (ex.: "Shazam" e "Patolino") usam o mesmo perfil,
-- ambos leem/escrevem no MESMO arquivo -> toda configuracao (Combo,
-- Especiais, Mystic, Stack, treinador, CaveBot, TargetBot, etc.) aparece
-- repetida entre contas diferentes.
--
...
local EXCLUDED_KEYS = {
    ...
    ["tyrFriendlist"] = true,
    -- NOTE: "_macros" (Attack Target, Escadas, Sense, Push, Bug Map, Follow
    -- RTS, Follow Normal, Aceitar/Invitar PT, etc.) costumava ficar de fora do isolamento,
    -- tratada como "estado interno do bot". Na pratica isso significa que,
    -- quando 2 bonecos usam o mesmo profile_N.json, o ligado/desligado das
    -- macros de um vaza pro outro -- e quando a conexao cai e reconecta, se
    -- o engine desliga alguma macro por seguranca, essa mudanca fica salva
...
-- nativo nunca mais grava nada pessoal -- so o nosso arquivo dentro de /shazam_scripts/
-- storage tem os dados de verdade. Logo depois devolvemos tudo pro
-- "storage" em memoria, pra o jogo continuar funcionando normalmente na
-- sessao atual.
...
-- Garante que salva tambem ao fechar o client.
