...
elseif type(Config) == "table" and type(Config.setup) == "function" then
  ...
  error("CaveBot config manager is unavailable")
end

config = configManager.setup("cavebot_configs", configWidget, "cfg", function(name, enabled, data)
  ...
  for i, child in ipairs(actions) do
    if i > index then
      ...
      if string.starts(text, "goto:") then
        ...
        if posz() == pos.z then
          local maxDist = tyrBot.storage.gotoMaxDistance
  ...
  for i=0,100 do
    ...
    if child then
      ...
      if string.starts(text, "goto:") then
        ...
        if posz() == pos.z then
          if distanceFromPlayer(pos) <= tyrBot.storage.gotoMaxDistance/2 then
...
CaveBot.getFirstWaypointBeforeLabel = function(label)
  if type(label) ~= "string" or label:trim() == "" then return false end
  label = "label:"..label
  label = label:lower()
  local actions = ui.list:getChildren()
  local index
...
CaveBot.setCurrentProfile = function(name)
  if not CaveBot.profileExists(name) then
    ...
    if not CaveBot.missingProfileWarnings[profileName] then
      ...
      warn("[JinWoo Scripts] Perfil CaveBot ausente: " .. profileName)
    end
    return false
  end
  CaveBot.setOff()
...
CaveBot.gotoLabel = function(label)
  -- FIX (erro no goto label): antes chamava label:lower() e child.value:lower()
  -- sem checar se o valor existia. Se a acao "goto label" ficasse com o campo
  -- vazio, ou se qualquer acao "label" na lista estivesse sem nome, o bot
  -- quebrava com erro Lua (attempt to call method 'lower' on a nil value)
  -- em vez de simplesmente avisar que o label nao foi encontrado.
  if type(label) ~= "string" or label:trim() == "" then
    warn("[CaveBot] Acao 'goto label' sem nome de label definido.")
    return false
  end
  label = label:lower()
  for index, child in ipairs(ui.list:getChildren()) do
    if child.action == "label" and type(child.value) == "string" and child.value:lower() == label then
